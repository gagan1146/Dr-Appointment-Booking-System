...
const DoctorProfile = () => {
  ...
  const updateProfile = async() => {
    try {
        ...
        console.log('dToken being sent:', dToken);
    ...
    <div>
        <div className='flex flex-col gap-4 m-5'>
            ...
            <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
                ...
                {/* -------- Doctor Info : name, degrre, experience ------- */}
...

                {
                    isEdit 
                    ?
                    <button onClick={updateProfile} className='px-4 py-1 border border-primary text-sm rounded-full mt-5 group-hover-bg hover:text-white transition-all'>Save</button>
                    :
                     <button onClick={() => setIsEdit(true)} className='px-4 py-1 border border-primary text-sm rounded-full mt-5 group-hover-bg hover:text-white transition-all'>Edit</button>
                }
